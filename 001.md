...
Luckily the Italian government puts a great deal of data about the COVID-19 epidemics on [Github](https://github.com/pcm-dpc/COVID-19), so I went on, downloaded the whole tresure trove, and started writing some code.
...
sub readfiles {
  my @files = dir('../../COVID-19/dati-regioni/', test => { .starts-with: 'dpc-covid19-ita-regioni-20' }).sort; # [¹]
  my ($date, @tests);
  for @files -> $file {
    state $prevnumber = 0;                                                                                      # [²]
...
my $w = array-vec(-> $vec { $mw.mean($vec, :endtype(GSL_MOVSTAT_END_PADVALUE)) }, @tests);    # [³]
...
Let's run the program and look at what I get now. This Gnuplot script shows the two datasets overlaid:
...
my constant $K = 7;
my Math::Libgsl::MovingWindow $mw .= new: :samples($K);
my $w = array-vec(-> $vec { $mw.mean($vec, :endtype(GSL_MOVSTAT_END_PADVALUE)) }, @new-cases);
...
Based on what I can see, in March and April the increase of tests followed the increase of new cases. In September the number of tests incresed while the new cases remained almost stable, then in November the number of new cases dropped while the number of tests continued raising.
I think the graph proves our doubts.
